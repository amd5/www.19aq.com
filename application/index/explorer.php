...
const COIN_NAME = 'zbtd';
const COIN_POS = false;
const RETURN_JSON = false;
const DATE_FORMAT = 'Y-m-d H:i:s';
const BLOCKS_PER_LIST = 1;
//const DB_FILE = 'db/somecoin_db.sq3';
const DB_FILE = false;
// for the example explorer
const COIN_HOME = '#';
const REFRESH_TIME = 180;
// courtesy of https://github.com/aceat64/EasyBitcoin-PHP/   https://github.com/stolendata/rpc-ace/
...
END;
// header
echo '<table><tr><td class="urgh"><b><a href="' . COIN_HOME . '" target="_blank">区块浏览器</a></b></td><td>当前区块:</td><td><a href="?' . $ace['num_blocks'] . '">' . $ace['num_blocks'] . '</a>';
$diffNom = '当前难度';
...
echo '<tr><td>Powered by <a href="https://github.com/stolendata/rpc-ace/" target="_blank">RPC Ace</a> v' . ACE_VERSION . ' (RPC AnyCoin Explorer)</td><td>网络算力: </td><td>' . $ace['hashrate_mhps'] . ' MH/s</td></tr><tr><td> </td><td></td><td></td></tr></table>';
// list of blocks
if( isset($ace['blocks']) )
{
    echo "<table><tr><td><b>区块</b></td><td><b>哈希值Hash</b></td><td><b>$diffNom</b></td><td><b>区块产生时间</b></td><td><b>交易笔数 &middot; 交易数量</b></td></tr><tr><td colspan=\"5\"></td></tr>";
    foreach( $ace['blocks'] as $block )
        echo "<tr class=\"illu\"><td>{$block['height']}</td><td><a href=\"{$block['hash']}\">" 
        . substr( $block['hash'], 0, 16 ) . 
        '&hellip;</a></td><td>' 
        . sprintf( '%.2f', $block['difficulty'] ) .
         "</td><td>{$block['date']}</td><td>{$block['tx_count']}----" 
        . sprintf( '%.2f', $block['total_out'] ) . '</td></tr>';
        $newer = $query < $ace['num_blocks'] ? '<a href="?' 
        . ( $ace['num_blocks'] - $query >= BLOCKS_PER_LIST ? $query + BLOCKS_PER_LIST : $ace['num_blocks'] ) .
         '">&lt; Newer</a>' : '&lt; Newer';
